...
const { fileExtensionMaker } = require('../helper/fileExtensionMaker')
...
const componentCreator = async (structure, parentDirectory, componentTemplate, language) => {
  ...
  const { name } = await inquirer.prompt([
    { name: "name", message: "Enter your screen name " },
  ]);
  ...
  await fs.writeFile(
    path.join(newScreenDirectory, `/${name}.${fileExtension}`),
    template
  );

  await fs.writeFile(
    path.join(newScreenDirectory, `/${name}.module.scss`),
    ""
  );
};
