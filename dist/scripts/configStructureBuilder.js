...
const configStructureBuilder = async () => {
  ...
  await fs.writeFile(path.join(destinationPath, '.structuregenerator.config.js'), configTemplate);
  const templateDirectoryPath = path.join(destinationPath, 'templates');
  await fs.mkdir(templateDirectoryPath);
  await fs.writeFile(path.join(templateDirectoryPath, 'components.md'), componentTemplate);
};
